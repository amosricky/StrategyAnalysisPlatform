import os , time
import pandas as pd
from SingleBackTestParameter import TheilsU , MAPE , WFE , Profit_Factor_Score , NetProfit_MDD_Score , isOutputFile
...
def ExecuteSingleBackTest(Strategy , Target , StartDay , EndDay , InSample_Length , OutSample_Length , Initial_Capital ,
                          Plateau_Condition , Plateau_Amount , SelectedFunction , PassPercent , PassMinimum):

    print("計算開始時間 : " + time.strftime("%H:%M:%S"))
    WalkForward = CreateWalkForwardDataFrame.WalkForwardDataFrame(Strategy , Target , StartDay , EndDay , InSample_Length , OutSample_Length , Initial_Capital)
    ...
    Longterm = CallLongterm.CallLongterm(Strategy , Target , Total_Period , StartDay , EndDay, InSample_Length , OutSample_Length ,
