...
class Longterm_RiskFactor_A:
    ...
    def Analysis(self):
        ...
        RiskFactorA_LongtermFunction = LongtermFunction.LongtermFunction(Target_Path, Start_Day, End_Day, Insample_Length_Month, Outsample_Length_Month)
        ...
        for i in range(1, LastPeriod + 1):
            ...
            SamplePeriod = RiskFactorA_LongtermFunction.CutTime(str(StartDay), End_Day)
            ...
            for j in range(0, len(SamplePeriod)):
                ...
                Training_Window_Start = RiskFactorA_LongtermFunction.StartDayChange(SamplePeriod[j][0][0])
                Training_Window_End = RiskFactorA_LongtermFunction.EndDayChange(SamplePeriod[j][0][1])
                Training_Window_Length = SamplePeriod[j][0][2]
                Testing_Window_Start_Day = RiskFactorA_LongtermFunction.StartDayChange(SamplePeriod[j][1][0])
                Testing_Window_End_Day = RiskFactorA_LongtermFunction.EndDayChange(SamplePeriod[j][1][1])
                ...
                for Filename in os.listdir(Target_Path):
                    ...
                    if(InSampleAvgNet>=Plateau_Condition):
                        ...
                        Net_Shifted = All_File_Data[Filename].loc[Training_Window_Start:Testing_Window_End_Day]['NetProfit'].copy().shift()
                        ...
                        #若獲利因子大於門檻
                        if((GrossProfit/abs(GrossLoss))>=Profit_Factor_Score):
                            CountPass += 1
