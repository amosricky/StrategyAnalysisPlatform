...
class Longterm_ReturnRate_E:
    def __init__(self, All_File_Data , Target_Path , Total_Period , Start_Day , End_Day, Insample_Length_Month , Outsample_Length_Month ,
                 Initial_Capital , Plateau_Condition , Longterm_Period , Longterm_Minimum , SelectedFunction , TheilsU , MAPE , WFE):
    ...
    def ReturnRateE(self, InSampleAvgTradeNet , OutSampleAvgTradeNet):
        ...
        if (SelectedFunction == 'MAPE'):
            ParameterScore = abs((OutSampleAvgTradeNet - InSampleAvgTradeNet) / OutSampleAvgTradeNet)
            Pass = ParameterScore <= MAPE

        elif (SelectedFunction == 'TheilsU'):
            ParameterScore = abs(OutSampleAvgTradeNet - InSampleAvgTradeNet) / (abs(InSampleAvgTradeNet) + abs(OutSampleAvgTradeNet))
            Pass = ParameterScore <= TheilsU

        elif (SelectedFunction == 'WFE'):
            ParameterScore = (OutSampleAvgTradeNet / InSampleAvgTradeNet)
            Pass = ParameterScore >= WFE
    ...
    def Analysis(self):
        ...
        ReturnRateE_LongtermFunction = LongtermFunction.LongtermFunction(Target_Path, Start_Day, End_Day, Insample_Length_Month, Outsample_Length_Month)
        ...
        for i in range(1, LastPeriod + 1):
            ...
            SamplePeriod = ReturnRateE_LongtermFunction.CutTime(str(StartDay), End_Day)
            ...
            for j in range(0, len(SamplePeriod)):
                ...
                Training_Window_Start = ReturnRateE_LongtermFunction.StartDayChange(SamplePeriod[j][0][0])
                Training_Window_End = ReturnRateE_LongtermFunction.EndDayChange(SamplePeriod[j][0][1])
                Training_Window_Length = SamplePeriod[j][0][2]
                Testing_Window_Start_Day = ReturnRateE_LongtermFunction.StartDayChange(SamplePeriod[j][1][0])
                Testing_Window_End_Day = ReturnRateE_LongtermFunction.EndDayChange(SamplePeriod[j][1][1])
                ...
                for Filename in os.listdir(Target_Path):
                    if (Training_Window_Start == 0):
                        ...
                        InSampleWindow = (All_File_Data[Filename][(All_File_Data[Filename].index >= Training_Window_Start) & (All_File_Data[Filename].index <= Training_Window_End)])
                        InSampleTranscation_Number = len(InSampleWindow[InSampleWindow['action'] == 'Buy'])
                        InSampleAvgTradeNet = (InSampleNumerator / InSampleDenominator) / InSampleTranscation_Number
                    ...
                    if(InSampleAvgNet>=Plateau_Condition):
                        ...
                        if (Training_Window_Start == 0):
                            ...
                            OutSampleWindow = (All_File_Data[Filename][(All_File_Data[Filename].index >= Testing_Window_Start_Day) & (All_File_Data[Filename].index <= Testing_Window_End_Day)])
                            OutSampleTranscation_Number = len(OutSampleWindow[OutSampleWindow['action'] == 'Buy'])
                            OutSampleAvgTradeNet = ( OutSampleNumerator / OutSampleDenominator) / OutSampleTranscation_Number
                        ...
                        #若該參數通過檢測，CountPass+1
                        if(self.ReturnRateE(InSampleAvgTradeNet,OutSampleAvgTradeNet)):
                            CountPass += 1
