...
class Longterm_ReturnRate_B:
    ...
    def ReturnRateB(self,InSampleAvgNet , OutSampleAvgNet):
        ...
        if (SelectedFunction == 'MAPE'):
            ParameterScore = abs((OutSampleAvgNet - InSampleAvgNet) / OutSampleAvgNet)
            Pass = ParameterScore <= MAPE

        elif (SelectedFunction == 'TheilsU'):
            ParameterScore = abs(OutSampleAvgNet - InSampleAvgNet) / (abs(InSampleAvgNet) + abs(OutSampleAvgNet))
            Pass = ParameterScore <= TheilsU

        elif (SelectedFunction == 'WFE'):
            ParameterScore = (OutSampleAvgNet / InSampleAvgNet)
    ...
    def Analysis(self):
        ...
        ReturnRateB_LongtermFunction = LongtermFunction.LongtermFunction(Target_Path, Start_Day , End_Day , Insample_Length_Month , Outsample_Length_Month)
        ...
        for i in range(1, LastPeriod + 1):
            ...
            SamplePeriod = ReturnRateB_LongtermFunction.CutTime(str(StartDay), End_Day)
            ...
            for j in range(0, len(SamplePeriod)):
                ...
                Training_Window_Start = ReturnRateB_LongtermFunction.StartDayChange(SamplePeriod[j][0][0])
                Training_Window_End = ReturnRateB_LongtermFunction.EndDayChange(SamplePeriod[j][0][1])
                Training_Window_Length = SamplePeriod[j][0][2]
                Testing_Window_Start_Day = ReturnRateB_LongtermFunction.StartDayChange(SamplePeriod[j][1][0])
                Testing_Window_End_Day = ReturnRateB_LongtermFunction.EndDayChange(SamplePeriod[j][1][1])
                ...
                for Filename in os.listdir(Target_Path):
                    ...
                    if(InSampleAvgNet>=Plateau_Condition):
                        if(self.ReturnRateB(InSampleAvgNet,OutSampleAvgNet)):
                            CountPass+=1
