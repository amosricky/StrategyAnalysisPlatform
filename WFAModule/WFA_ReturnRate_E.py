...
class WFA_ReturnRate_E:
    ...
    def TraceAll_ReturnRate_E_CalculatePass(self,InSampleNetAvgTrade, OutSampleNetAvgTrade):
        ...
        if (SelectedFunction == 'MAPE'):
            ParameterScore = abs((OutSampleNetAvgTrade - InSampleNetAvgTrade) / OutSampleNetAvgTrade)
            isWindowPass = ParameterScore <= MAPE

        elif (SelectedFunction == 'TheilsU'):
            ParameterScore = abs(OutSampleNetAvgTrade - InSampleNetAvgTrade) / (abs(InSampleNetAvgTrade) + abs(OutSampleNetAvgTrade))
            isWindowPass = ParameterScore <= TheilsU

        elif (SelectedFunction == 'WFE'):
            ParameterScore = (OutSampleNetAvgTrade / InSampleNetAvgTrade)
    ...
    def OutputFile(self , Table):
        ...
        if(isOutputFile == True):
            ...
            Table.to_csv(Target_DirPath+'//WFA_報酬率E_各窗格結果.csv', mode='a',header=True, index=False)
