...
class TraceAll_RiskFactor_B:
    ...
    def OutputFile(self, Table, Statitic):
        ...
        if (isOutputFile == True):
            ...
            # Table.to_csv(Target_DirPath + '//TraceAll_風險因子B_各窗格結果.csv', mode='a', header=True, index=False)
            Statitic.to_csv(Target_DirPath + '//TraceAll_風險因子B_整體結果.csv', mode='a', header=True, index=False)
