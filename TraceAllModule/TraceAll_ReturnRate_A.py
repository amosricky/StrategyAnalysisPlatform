...
class TraceAll_ReturnRate_A:
    ...
    def Analysis(self):
        ...
        for Period in range(1, Total_Period + 1):
            ...
            if (Table['Para' + str(Period)][0] == 'No Plateau'):
                ...
                ThisPeriodFrame = pd.DataFrame({'AllPara': Table['Para' + str(Period)], 'Pass' + str(Period): Table['Pass' + str(Period)]})
    ...
    def OutputFile(self, Table , Statitic):
        ...
        if (isOutputFile == True):
            ...
            # Table.to_csv(Target_DirPath + '//TraceAll_報酬率A_各窗格結果.csv', mode='a', header=True, index=False)
            Statitic.to_csv(Target_DirPath + '//TraceAll_報酬率A_整體結果.csv', mode='a', header=True, index=False)
