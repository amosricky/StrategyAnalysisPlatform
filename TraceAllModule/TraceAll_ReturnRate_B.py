...
class TraceAll_ReturnRate_B:
    ...
    def TraceAll_ReturnRate_B_CalculatePass(self,InSamepleNet, OutSampleNet):
    ...
    def Analysis(self):
        ...
        for Period in range(1, Total_Period + 1):
            ...
            if (len(isOverCondition[isOverCondition == True]) > 0):
                ...
                # 判斷該窗格推薦參數是否合格
                Score, Pass = self.TraceAll_ReturnRate_B_CalculatePass(InSamepleNet, OutSampleNet)
    ...
    def OutputFile(self, Table, Statitic):
        ...
        if (isOutputFile == True):
            ...
            # Table.to_csv(Target_DirPath + '//TraceAll_報酬率B_各窗格結果.csv', mode='a', header=True, index=False)
            Statitic.to_csv(Target_DirPath + '//TraceAll_報酬率B_整體結果.csv', mode='a', header=True, index=False)
